...
const StatusBadge = ({ status }) => {
    const map = {
        ...
        'in-review':   { cls: 'bg-gradient-to-r from-blue-50 to-indigo-50 text-blue-700 border-blue-200', icon: Eye, label: 'In Review' },
        ...
        resolved:      { cls: 'bg-gradient-to-r from-green-50 to-emerald-50 text-green-700 border-green-200', icon: CheckCircle, label: 'Resolved' },
        dismissed:     { cls: 'bg-gradient-to-r from-gray-50 to-gray-100 text-gray-500 border-gray-200', icon: XCircle, label: 'Dismissed' },
        blocked:       { cls: 'bg-gradient-to-r from-red-50 to-rose-50 text-red-700 border-red-200', icon: ShieldX, label: 'Blocked' },
    };
    ...
    const ACTIONS = [
        {
            ...
            activeClass: 'border-yellow-500 bg-gradient-to-r from-yellow-50 to-amber-50',
        ...
        },
        {
            ...
            activeClass: 'border-red-500 bg-gradient-to-r from-red-50 to-rose-50',
        ...
        },
        {
            ...
            activeClass: 'border-green-500 bg-gradient-to-r from-green-50 to-emerald-50',
        ...
        },
        {
            ...
            activeClass: 'border-gray-400 bg-gradient-to-r from-gray-50 to-gray-100',
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-orange-100 text-xs">
                                Against: <strong className="text-white">{worker?.name}</strong> · {worker?.karigarId}
                            </p>
                        </div>
                        <motion.button
...
// ── Enhanced Worker Profile Drawer ────────────────────────────────────────────
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {[
                                { label: 'Mobile', value: worker.mobile, icon: Phone, color: 'orange' },
                                { label: 'Experience', value: worker.experience ? `${worker.experience} yrs` : 'N/A', icon: Briefcase, color: 'blue' },
                                { label: 'Status', value: worker.verificationStatus, icon: ShieldCheck, color: 'green' },
                                { label: 'Points', value: worker.points ?? '—', icon: Award, color: 'purple' },
                                { label: 'Gender', value: worker.gender || 'N/A', icon: User, color: 'gray' },
                                { label: 'City', value: worker.address?.city || 'N/A', icon: MapPin, color: 'orange' },
                            ].map(({ label, value, icon: Icon, color }) => (
                                <div key={label} className={`bg-gradient-to-r from-${color}-50 to-${color}-100/30 rounded-xl p-3 border border-${color}-200`}>
                    ...
                    {skills.length > 0 && (
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {skills.map((s, i) => (
                                    <span key={i} className="bg-gradient-to-r from-orange-100 to-amber-100 text-orange-700 text-xs font-medium px-3 py-1.5 rounded-full border border-orange-200">
                                        {s}
                                    </span>
                                ))}
                            </div>
                    ...
                    {complaint.adminNotes && (
                        <motion.div
                            ...
                            className={`rounded-2xl p-4 border ${
                                complaint.adminAction === 'blocked'   ? 'bg-gradient-to-r from-red-50 to-rose-50 border-red-200' :
                                complaint.adminAction === 'warned'    ? 'bg-gradient-to-r from-yellow-50 to-amber-50 border-yellow-200' :
                                complaint.adminAction === 'cleared'   ? 'bg-gradient-to-r from-green-50 to-emerald-50 border-green-200' :
                        ...
                        >
                            ...
                            <p className="text-sm text-gray-800 leading-relaxed">{complaint.adminNotes}</p>
                    ...
                    <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl p-4 border border-gray-200">
                        ...
                        <a
                            ...
                            className="flex items-center justify-center gap-2 w-full py-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-2xl font-bold text-sm shadow-lg shadow-green-200 hover:shadow-green-300 transition-all hover:scale-105 active:scale-95"
                        >
                            <Phone size={16} />
                ...
                {!isActioned && (
                    <div className="flex-shrink-0 p-5 border-t border-gray-100 bg-white">
                        <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            onClick={onAction}
                            className="w-full py-4 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-2xl font-black shadow-lg shadow-orange-200 hover:shadow-orange-300 transition-all flex items-center justify-center gap-2"
                        >
...
export default function AdminComplaints() {
    ...
    const STATUS_FILTERS = [
        { id: 'all',          label: 'All',          count: stats.total || 0, icon: BarChart3, gradient: 'from-gray-500 to-gray-600', color: 'gray' },
        { id: 'submitted',    label: 'New',          count: stats.submitted || 0, icon: Clock, gradient: 'from-yellow-500 to-amber-500', color: 'yellow' },
        { id: 'in-review',    label: 'In Review',    count: stats.inReview || 0, icon: Eye, gradient: 'from-blue-500 to-indigo-500', color: 'blue' },
        { id: 'action-taken', label: 'Action Taken', count: stats.actionTaken || 0, icon: Flag, gradient: 'from-orange-500 to-red-500', color: 'orange' },
        { id: 'resolved',     label: 'Resolved',     count: stats.resolved || 0, icon: CheckCircle, gradient: 'from-green-500 to-emerald-500', color: 'green' },
        { id: 'dismissed',    label: 'Dismissed',    count: stats.dismissed || 0, icon: XCircle, gradient: 'from-gray-500 to-gray-600', color: 'gray' },
        { id: 'blocked',      label: 'Blocked',      count: stats.blocked || 0, icon: ShieldX, gradient: 'from-red-500 to-rose-500', color: 'red' },
    ];
                                    ...
                                    >
                                            ...
                                            <div className="flex flex-col md:flex-row md:items-start gap-4">
                                                ...
                                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                                    <div className="relative flex-shrink-0">
                                                        <img
                                                            src={worker?.photo || `https://ui-avatars.com/api/?name=${encodeURIComponent(worker?.name || 'W')}&background=f97316&color=fff&bold=true`}
                                                            alt={worker?.name}
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center gap-2 flex-wrap mb-1.5">
                                                        <span className="bg-gradient-to-r from-red-50 to-rose-50 text-red-700 text-xs font-bold px-2.5 py-1 rounded-full border border-red-200">
                                                            {c.category}
                                                        </span>
                                        ...
                                        {c.adminNotes && (
                                            <div className={`px-5 py-2.5 border-t text-xs ${
                                                c.adminAction === 'blocked'  ? 'bg-gradient-to-r from-red-50 to-rose-50 border-red-100 text-red-700' :
                                                c.adminAction === 'warned'   ? 'bg-gradient-to-r from-yellow-50 to-amber-50 border-yellow-100 text-yellow-700' :
                                                c.adminAction === 'cleared'  ? 'bg-gradient-to-r from-green-50 to-emerald-50 border-green-100 text-green-700' :
                                                'bg-gradient-to-r from-gray-50 to-gray-100 border-gray-100 text-gray-600'
                                            }`}>
