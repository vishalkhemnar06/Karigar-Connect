...
const getBlockedWindow = (job) => {
    ...
    const dur = parseDurationToHours(job.duration);
...
const checkWorkerScheduleConflict = async (JobModel, workerId, jobId) => {
    const target = await JobModel.findById(jobId).lean();
    if (!target) return { conflict: false };
    const tw = getBlockedWindow(target);
