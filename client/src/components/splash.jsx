  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-100 to-amber-50 overflow-hidden relative flex flex-col items-center justify-center p-4">
      ...
      <div className="relative z-20 text-center max-w-sm w-full animate-card-enter">
        ...
        <div className="mb-6">
          <div className="w-24 h-24 mx-auto mb-4 rounded-full bg-gradient-to-br from-orange-300 to-amber-300 shadow-lg flex items-center justify-center border-4 border-white/80 animate-logo-spin">
            <div className="w-18 h-18 rounded-full bg-white/90 flex items-center justify-center shadow-inner">
              <img 
                src={logo} 
                alt="KarigarConnect Logo" 
                className="w-14 h-14 object-contain rounded-full animate-logo-bounce"
              />
            </div>
          </div>
          
          {/* Company Name */}
          <h1 className="text-3xl font-black text-orange-800 mb-2 tracking-tight animate-typewriter glow-text">
            KarigarConnect
          </h1>
        </div>

        {/* Tagline with Staggered Words */}
        <p className="text-lg font-medium text-orange-700/90 mb-6 leading-relaxed">
        ...
        {/* Feature Icons */}
        <div className="flex justify-center space-x-3 mb-6 animate-fade-in-up animation-delay-800">
          {['🛠️', '🎨', '⭐', '🤝'].map((icon, index) => (
            <div
              key={index}
              className="w-10 h-10 bg-white/50 backdrop-blur-sm rounded-xl flex items-center justify-center border border-orange-200/50 shadow-sm animate-bounce-slow"
        ...
        <div className="mb-4">
          <div className="h-1.5 bg-white/40 rounded-full overflow-hidden mb-1 backdrop-blur-sm">
          ...
          <div className="flex justify-between text-sm text-orange-700/80">
            ...
            <span className="text-orange-600 font-semibold animate-pulse">1s</span>
          </div>
        </div>
        ...
        {/* Subtle Footer */}
        <p className="text-orange-600/70 text-xs animate-text-glow font-medium">
          Crafting Dreams into Reality
        </p>
      </div>

      {/* Custom Styles - All durations reduced */}
      <style>{`
        ...
        /* Glow Text Effect */
        @keyframes text-glow {
          0%, 100% { text-shadow: 0 0 10px rgba(255, 165, 0, 0.3); }
          50% { text-shadow: 0 0 15px rgba(255, 165, 0, 0.5); }
        }

        /* Word Fade Effects - Faster */
        ...
        .animate-word-fade-1 {
          animation: word-fade-1 0.8s ease-out forwards;
        }

        .animate-word-fade-2 {
          animation: word-fade-2 1s ease-out forwards;
        }

        .animate-word-fade-3 {
          animation: word-fade-3 1.2s ease-out forwards;
        }

        .animate-word-fade-4 {
          animation: word-fade-4 1.4s ease-out forwards;
        }
        ...
        .animate-move-line-1 {
          animation: move-line-1 3s linear infinite;
        }

        .animate-move-line-2 {
          animation: move-line-2 2.5s linear infinite;
          animation-delay: 0.5s;
        }

        .animate-move-line-3 {
          animation: move-line-3 3.5s linear infinite;
          animation-delay: 1s;
        }
